...
/** default size is 'sm' */
type TokenAvatarPairProps = RawTokenAvatarPairProps &
  Omit<FlexProps, keyof RawTokenAvatarPairProps>;
...
export default function TokenAvatarPair({
  ...
  const marginLeft = Array.isArray(size)
    ? size.map((s) => parseSize(s))
    : parseSize(size);
