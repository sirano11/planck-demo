...
import useResizeObserver from '@/raydium/hooks/useResizeObserver';
import { shrinkToValue } from '@/raydium/utils/shrinkToValue';
...
type TabItem = TabOptionsObj | string;

type TabsProps = Omit<
  _TabsProps,
  'children' | 'onChange' | 'index' | 'defaultIndex'
> & {
...
export default function Tabs({
  ...
  const defaultValueIndex = useMemo(() => {
    ...
    const hasSetDefaultValueInItems = options.some(
  ...
  const [forceRerenderIndicatorCount, setForceRerenderIndicatorCount] =
    useState(0);
  ...
  useEffect(() => {
    ...
    return () =>
      globalThis.document.removeEventListener('resize', forceRerenderIndicator);
  }, []);
  const indicatorLeft = useMemo(
    () => indicatorTarget?.offsetLeft,
    [indicatorTarget, forceRerenderIndicatorCount],
  );
  const indicatorWidth = useMemo(
    () => indicatorTarget?.offsetWidth,
    [indicatorTarget, forceRerenderIndicatorCount],
  );
  return (
    <_Tabs
    ...
    >
      ...
      >
        {options.map((option, idx) => (
          ...
          >
            {renderItem?.(option.value, idx) ??
              shrinkToValue(option.label, [activeIndex === idx]) ??
              option.value}
          </CustomTab>
        ))}
        {haveIndicator && (
