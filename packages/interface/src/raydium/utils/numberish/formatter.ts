...
export const trimTrailZero = (numberStr?: string) => {
  ...
  while (splitNum[1][splitNum[1].length - 1] === '0')
    splitNum[1] = splitNum[1].slice(0, -1);
...
export const isIntlNumberFormatSupported =
...
export function formatToRawLocaleStr(
  ...
  return decimalSeparator !== '.'
    ? val.toString().replace('.', decimalSeparator)
    : val;
}
...
function formatSmallNumberWithFixed(
  number: number,
  fixedDigits: number,
): number {
...
function formatWithAbbreviation(value: number, numDecimals: number) {
  ...
  for (let i = 0; i < thresholds.length; i++) {
    if (value >= thresholds[i].value) {
      const abbreviatedValue = trimTrailZero(
        (value / thresholds[i].value).toFixed(numDecimals),
      );
...
function generateFallbackFormatter(
  ...
  return {
    format: (value: number): string => {
      const formattedValue = abbreviated
        ? formatWithAbbreviation(value, numDecimals)
        : formatLocaleStr(value, numDecimals);
...
export function formatCurrency(
  ...
  const amountDecimal =
    amount instanceof Decimal
      ? amount
      : new Decimal(String(amount).replace(/,/gi, ''));
  ...
  if (noDecimal === true && amountNumber > 1) {
    return formatCurrencyOverride(
      currencyFormatterNoDecimal.format(amountNumber),
    );
  }
