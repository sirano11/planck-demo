...
import {
  extractNumberOnly,
  numberRegExp,
} from '@/raydium/utils/numberish/regex';
...
function DecimalInput(props: Props) {
  ...
  const shakeValueDecimal = (
    ...
    decimals != null &&
    new Decimal(extractNumberOnly(value, 0)).decimalPlaces() > decimals
      ? new Decimal(value)
          .toDecimalPlaces(decimals, Decimal.ROUND_FLOOR)
          .toString()
      : value;

  // shaked decimal
  const showedValue = useMemo(
    () => shakeValueDecimal(value, decimals),
    [value, decimals],
  );
  return (
    <Flex
