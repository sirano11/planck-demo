...
interface Props {
  onPriceChange?: (
    val: { current: number; change: number } | undefined,
  ) => void;
  // baseMint?: ApiV3Token;
...
export default function CandleChart({
  ...
  const pair = 'SOL-MEME';
  // baseMint && quoteMint
  //   ? `${baseMint?.address}-${quoteMint?.address}`
  //   : undefined;
  timeTypeRef.current = timeType;

  // const { data, currentPrice, change24H, isLoading, isEmptyResult, loadMore } =
  //   useFetchPoolKLine({
  ...
  useEffect(() => {
    ...
    const candlestickSeries = chart.addCandlestickSeries({
      ...
      priceFormat: {
        ...
        formatter: (val: number) => {
          return val
            ? formatCurrency(val, { maximumDecimalTrailingZeroes: 5 })
            : val;
        },
    ...
    setTimeout(() => {
      chart
        .timeScale()
        .subscribeVisibleLogicalRangeChange((newVisiableLogicalRange) => {
